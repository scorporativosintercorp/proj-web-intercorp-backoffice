<template>
  <div class="form--main">
    <form
      ...
      @submit.prevent="register(activo)"
    >
        ...
        <div class="p-6 bg-white rounded-md shadow">
          <div class="flex justify-between pb-4 border-b border-gray-100 mb-7">
            ...
            <div class="flex items-center">
              ...
              <select
                name="lang"
                class="py-1 pl-1 pr-8 leading-none border-none"
                @change="toggleLangs"
              >
          ...
          <div class="grid grid-cols-1 gap-6">
            <div v-if="cur_lang === 'es'">
              ...
              <input
                class="w-full text-xl border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                type="text"
                id="titulo"
                placeholder="Añadir título"
                v-model="titulo"
              />
              <div v-if="tituloError" class="label-error">
                {{ tituloError }}
              </div>
            </div>

            <div v-else-if="cur_lang === 'en'">
              <label class="sr-only" for="titulo_en"
                >Añadir título en inglés</label
              >
              <input
                class="w-full text-xl border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                type="text"
                id="titulo_en"
                placeholder="Añadir título en inglés"
                v-model="titulo_en"
              />
            </div>

            <div v-if="cur_lang === 'es'">
              <label class="text-gray-700 sr-only" for="bajada_nota"
                >Añadir bajada</label
              >
              <textarea
                class="w-full border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                rows="3"
                id="bajada_nota"
                placeholder="Teclea para ingresar la bajada de la nota"
                v-model="bajada_nota"
              ></textarea>
              <div v-if="bajadaNotaError" class="label-error">
                {{ bajadaNotaError }}
              </div>
            </div>

            <div v-if="cur_lang === 'en'">
              <label class="text-gray-700 sr-only" for="bajada_nota_en"
                >Añadir bajada en inglés</label
              >
              <textarea
                class="w-full border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                rows="3"
                id="bajada_nota_en"
                placeholder="Teclea para ingresar la bajada de la nota en inglés"
                v-model="bajada_nota_en"
              ></textarea>
            </div>
            ...
            <div v-if="cur_lang === 'es'">
              ...
              <input
                ...
                type="text"
                id="palabras_clave"
                placeholder="Ingresa las palabras claves de la nota"
                v-model="palabras_clave"
              />
            </div>

            <div v-if="cur_lang === 'en'">
              <label class="sr-only" for="palabras_clave_en"
                >Palabras clave en inglés</label
              >
              <input
                class="w-full border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                type="text"
                id="palabras_clave_en"
                placeholder="Ingresa las palabras claves de la nota en inglés"
                v-model="palabras_clave_en"
              />
            </div>
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
              <div v-if="cur_lang === 'es'" class="bg-white rounded-md shadow">
                <div class="px-6 py-4 border-b border-gray-200">
                  <h3 class="text-sm font-semibold">
                    Inserta la imagen de la cabecera de la nota
                  </h3>
                  <p class="text-sm">
                    Tamaño de la imagen: 960px x 540px a 72ppp.
                  </p>
                </div>
                <div class="p-6">
                  <input
                    type="file"
                    id="link_imagen_cabecera"
                    v-on:change="
                      subirImagen(
                        $event,
                        'link_imagen_cabecera',
                        'preview_image_header_url'
                      )
                    "
                    class="form-control"
                    placeholder="Imagen de la cabecera"
                  />
                  <div v-if="imagenCabeceraError" class="label-error">
                    {{ imagenCabeceraError }}
                  </div>
                </div>
              </div>
              ...
              <div v-if="cur_lang === 'en'" class="bg-white rounded-md shadow">
                <div
                  class="flex justify-between px-6 py-4 border-b border-gray-200"
                >
                  <h3 class="text-sm font-semibold">
                    Inserta la imagen de la cabecera de la nota en inglés
                  </h3>
                  <p class="text-sm">
                    Tamaño de la imagen: 960px x 540px a 72ppp.
                  </p>
                ...
                <div class="p-6">
                  <input
                    type="file"
                    id="link_imagen_cabecera_en"
                    v-on:change="
                      subirImagen(
                        $event,
                        'link_imagen_cabecera_en',
                        'preview_image_header_url_en'
                      )
                    "
                    class="form-control"
                    placeholder="Imagen Preview"
                  />
                </div>
              </div>
              ...
              <div class="bg-white rounded-md shadow">
                ...
                <div class="p-6">
                  <figure v-if="cur_lang === 'es'">
                    <img
                      v-if="preview_image_header_url"
                      :src="preview_image_header_url"
                      alt="Imagen previa de la cabecera"
                    />
                  </figure>
                  <figure v-if="cur_lang === 'en'">
                    <img
                      v-if="preview_image_header_url_en"
                      :src="preview_image_header_url_en"
                      alt="Imagen previa de la cabecera"
                    />
                  </figure>
                </div>
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
              <div v-if="cur_lang === 'es'" class="bg-white rounded-md shadow">
                <div class="px-6 py-4 border-b border-gray-200">
                  <h3 class="text-sm font-semibold">
                    Inserta la imagen previa de la nota
                  </h3>
                  <p class="text-sm">
                    Tamaño de la imagen: 400px x 266px a 72ppp.
                  </p>
                </div>
                <div class="p-6">
                  <input
                    type="file"
                    id="link_imagen_preview"
                    v-on:change="
                      subirImagen(
                        $event,
                        'link_imagen_preview',
                        'preview_image_preview_url'
                      )
                    "
                    class="form-control"
                    placeholder="Imagen Preview"
                  />
                  <div v-if="imagenPreviaError" class="label-error">
                    {{ imagenPreviaError }}
                  </div>
                </div>
              </div>
              ...
              <div v-if="cur_lang === 'en'" class="bg-white rounded-md shadow">
                <div class="px-6 py-4 border-b border-gray-200">
                  <h3 class="text-sm font-semibold">
                    Inserta la imagen previa de la nota en inglés
                  </h3>
                  <p class="text-sm">
                    Tamaño de la imagen: 400px x 266px a 72ppp.
                  </p>
                ...
                <div class="p-6">
                  <input
                    type="file"
                    id="link_imagen_preview_en"
                    v-on:change="
                      subirImagen(
                        $event,
                        'link_imagen_preview_en',
                        'preview_image_preview_url_en'
                      )
                    "
                    class="form-control"
                    placeholder="Imagen Preview"
              ...
              <div class="bg-white rounded-md shadow">
                ...
                <div class="flex flex-col items-center p-6">
                  <figure v-if="cur_lang === 'es'">
                    <img
                      :src="preview_image_preview_url"
                      width="400"
                      v-if="preview_image_preview_url"
                      alt="Imagen previa de la cabecera"
                    />
                  </figure>
                  <figure v-if="cur_lang === 'en'">
                    <img
                      :src="preview_image_preview_url_en"
                      v-if="preview_image_preview_url_en"
                      width="400"
                      alt="Imagen previa de la cabecera"
                    />
                  </figure>
      ...
      <p class="mb-4 font-bold text-red-700">
        Datos compartidos entre español e inglés:
      </p>
      ...
      <div class="mb-10">
        <div class="p-6 bg-white rounded-md shadow">
              ...
              <div class="bg-[#F4F8FD] rounded-md">
                <div class="p-6">
                  <div class="grid grid-cols-1 gap-5">
                    <div>
                      <label class="sr-only" for="autor_nombre"
                        >Ingresa el nombre del autor</label
                      >
                      <input
                        class="w-full border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                        type="text"
                        ...
                        v-model="autor_nombre"
                      />
                    </div>

                    <div>
                      <label class="sr-only" for="autor_cargo"
                        >Ingresa el cargo del autor</label
                      >
                      <input
                        class="w-full border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                        type="text"
                        id="autor_cargo"
                        placeholder="Ingresa el cargo del autor"
                        v-model="autor_cargo"
                      />
                    </div>
                    ...
                    <div>
                      <label class="block mb-4 leading-none" for="autor_imagen"
                        ...
                        <span class="text-sm"
                          >Tamaño de la imagen: 80px x 80px a 72ppp.</span
                        >
                      </label>
                      <input
                        type="file"
              ...
              <div class="bg-white rounded-md shadow">
                ...
                <div class="p-6">
                  <div class="flex justify-center">
                    <figure>
                      <img
                        :src="preview_image_author_url"
                        v-if="preview_image_author_url"
                        width="80"
          ...
          <div class="pb-4 border-b border-gray-100 mb-7">
            ...
            <p class="text-sm">
              Copiar y pegar este texto: <strong>###quote###</strong> en el
              cuerpo de la noticia donde se quiera mostrar esta cita
            </p>
          </div>
            ...
            <div>
              <div class="bg-[#F4F8FD] rounded-md">
                <div class="p-6">
                  <div class="grid grid-cols-1 gap-5">
                    <div>
                      <label class="sr-only" for="cita_autor"
                        >Ingresa el nombre del autor de la cita</label
                      >
                      <input
                        class="w-full border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                        type="text"
                        id="cita_autor"
                        placeholder="Ingresa el nombre del autor de la cita"
                        v-model="cita_autor"
                      />
                    </div>

                    <div>
                      <label class="sr-only" for="cita_cargo"
                        >Ingresa el cargo del autor de la cita</label
                      >
                      <input
                        class="w-full border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                        type="text"
                        id="cita_cargo"
                        placeholder="Ingresa el cargo del autor de la cita"
                        v-model="cita_cargo"
                      />
                    </div>

                    <div>
                      <label class="sr-only" for="cita_parrafo_principal"
                        >Ingresa el párrafo principal de la cita</label
                      >
                      <textarea
                        class="w-full border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                        rows="4"
                        type="text"
                        id="cita_parrafo_principal"
                        placeholder="Ingresa el párrafo principal de la cita"
                        v-model="cita_parrafo_principal"
                      ></textarea>
                    </div>

                    <div>
                      <label class="sr-only" for="cita_parrafo_secundario"
                        >Ingresa el párrafo secundario de la cita</label
                      >
                      <textarea
                        class="w-full border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                        rows="4"
                        type="text"
                        id="cita_parrafo_secundario"
                        placeholder="Ingresa el párrafo secundario de la cita"
                        v-model="cita_parrafo_secundario"
                      ></textarea>
                    </div>
                    ...
                    <div>
                      <label class="block mb-4 leading-none" for="cita_imagen"
                        >Inserta la imagen del autor<br />
                        <span class="text-sm"
                          >Tamaño de la imagen: 160px x 160px a 72ppp.</span
                        >
                      </label>
                      <input
                        type="file"
                        ...
                        v-on:change="
                          subirImagen(
                            $event,
                            'cita_imagen',
                            'preview_image_quote_author_url'
                          )
                        "
                        class="form-control"
                        placeholder="Imagen Preview"
            ...
            <div>
              <div class="bg-white rounded-md shadow h-full">
                <div
                  class="flex justify-between px-6 py-4 border-b border-gray-200"
                >
                  <h3 class="text-sm font-semibold">Vista Previa de la Cita</h3>
                </div>
                <div
                  class="skew-bg flex px-8 py-14 bg-violet text-white skew-y-[-10deg]"
                  :style="{ backgroundColor: color_principal }"
                >
                  <div class="flex gap-5 items-center h-full skew-y-[10deg]">
                    <div class="w-32 flex-none flex flex-col gap-3">
                      <figure
                        class="w-full"
                        v-if="preview_image_quote_author_url"
                      >
                        <img
                          :src="preview_image_quote_author_url"
                      ...
                      <div
                        v-else
                        class="w-24 h-24 flex-none bg-gray-100 rounded-full flex items-center justify-center text-gray-800 border-2 border-dashed border-gray-800"
                      >
                      ...
                      <div class="text-left">
                        ...
                        <div v-else class="font-bold text-lg leading-none">
                          Nombre del Autor
                        </div>
                        <div class="text-md" v-if="cita_cargo">
                          {{ cita_cargo }}
                        </div>
                        <div v-else class="text-md">Cargo</div>
                      </div>
                    </div>

                    <div class="flex-1">
                      <blockquote
                        class="italic flex-1 text-left"
                        v-if="cita_parrafo_principal || cita_parrafo_secundario"
                      >
                        <p
                          class="mb-3 text-lg font-medium leading-tight"
                          v-if="cita_parrafo_principal"
                        >
                          "{{ cita_parrafo_principal }}"
                        </p>
                        <p class="text-sm" v-if="cita_parrafo_secundario">
                          {{ cita_parrafo_secundario }}
                        </p>
                      </blockquote>
                      <div v-else class="text-white italic">
                        "El texto de la cita aparecerá aquí..."
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="grid grid-cols-1 gap-7">
                    <div>
                      ...
                      <input
                        ...
                        type="text"
                        id="seo_meta_title"
                        placeholder="Intercorp - Título de la noticia"
                        v-model="seo_meta_title"
                      />
                    </div>

                    <div>
                      <label class="" for="seo_meta_description"
                        >Meta Description</label
                      >
                      <textarea
                        class="w-full border-gray-200 rounded-md ext-xl focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                        rows="3"
                        id="seo_meta_description"
                        placeholder="Ingresar la meta description"
                        v-model="seo_meta_description"
                      ></textarea>
                    </div>

                    <div>
                      <label class="" for="seo_meta_keywords"
                        >Meta Keywords</label
                      >
                      <input
                        class="w-full border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                        ...
                        placeholder="Ejemplo: palabra clave 1, palabra clave 2, palabra clave 3"
                        v-model="seo_meta_keywords"
                      />
                    </div>
                  </div>
                </div>
          ...
          <div class="mb-7">
            ...
            <p class="text-sm">
              Copiar y pegar este texto: <strong>###gallery###</strong> en el
              cuerpo de la noticia donde se quiera mostrar la galería de
              imágenes.
            </p>
          </div>
          ...
          <div class="grid grid-cols-1 gap-5 mb-10 md:grid-cols-2">
            <div class="bg-white rounded-md shadow">
              ...
              >
                <h3 class="text-sm font-semibold">
                  Inserta las imágenes para la galería
                </h3>
              </div>
              <div class="p-6">
                <div>
                  <div
                    ...
                    class="cursor-pointer gallery-images--add"
                  >
                ...
                <div class="gallery--grid">
                  ...
                  >
                    <div class="gallery--content">
                      <button
                        type="button"
                        @click="eliminarImagenGaleria(item.id, key)"
          ...
          <div class="bg-[#F4F8FD] rounded-md mb-8">
            <div class="p-6">
              ...
              <div class="grid grid-cols-1 gap-7">
                <div>
                  <label class="sr-only" for="tiempo_de_lectura"
                    >Meta Tittle</label
                  >
                  <input
                    class="w-full border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
                    type="text"
                    id="tiempo_de_lectura"
                    placeholder="Colocar el texto en minutos."
                    v-model="tiempo_de_lectura"
                  />
                </div>
              </div>
            </div>
      ...
      <p class="mb-4 font-bold text-red-700">
        Elige la ubicación del texto dentro de la web:
      </p>
                ...
                <li>
                  <label class="radio" for="tipo-noticia">
                    <input
                      ...
                      id="tipo-noticia"
                      value="0"
                      v-model="tipo_noticia"
                      @change="setNewsType($event)"
                    />
                    <span>Noticia</span>
                  </label>
                </li>
                ...
                <li>
                  <label class="radio" for="tipo-comunicados-sorporativos">
                    <input
                      type="radio"
                      name="tipo_noticia"
                      id="tipo-comunicados-sorporativos"
                      value="3"
                      v-model="tipo_noticia"
                      @change="setNewsType($event)"
                    />
      ...
      <div class="flex justify-end mb-10">
        <button @click="preview" type="button" class="btn btn-blue">
          Previsualizar
        </button>
    ...
    <div v-if="sent" class="h-[50vh] flex flex-col justify-center items-center">
      <div class="form--response__content">
        <div class="w-full max-w-sm p-6 bg-white rounded-md shadow-md">
          <div class="mb-5">
            <h2 class="font-bold text-center stext-2xl">
              Noticia guardada correctamente
            </h2>
          </div>
          <div class="text-center">
            <router-link to="/noticias" class="btn btn-red"
              ><i class="mr-2 fa fa-arrow-left"></i> Volver</router-link
            >
          </div>
        </div>
      </div>
    ...
    <div class="modale--block" v-if="revele">
      ...
      <div class="modale">
        <h2 class="mb-6 font-semibold text-center">
          Agregar nueva imagen a la galería
        </h2>
        <div class="mb-4">
          <input
            class="w-full text-sm border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"
            type="text"
            placeholder="Ingresa la leyenda de la imagen"
            id="legend"
          />
        </div>
        <div class="mb-4">
        ...
        <div class="flex justify-between">
          <button type="button" @click="toggleModale" class="btn btn-red">
            Cancelar
          </button>
          <button
            type="button"
            @click="addItemGaleria"
            :disabled="isDisabled"
            class="btn btn-blue"
          >
            Agregar
  ...
  computed: {
    editorConfig() {
      return {
        ...
        toolbar:
          'undo redo | blocks | link image media | bold italic underline backcolor | \
          alignleft aligncenter alignright alignjustify | blockquote | \
          bullist numlist outdent indent | removeformat | help | code preview columns',
        relative_urls: false,
        convert_urls: true,
        extended_valid_elements:
          'iframe[src|title|width|height|allowfullscreen|allow|referrerpolicy|class|id]',
        media_live_embeds: true,
        image_title: true,
        images_upload_url: this.API_URL + 'subirimagen-richtext',
        images_upload_handler: this.uploadImagenRichText,
        content_style: `
        ...
        setup: function (editor) {
          editor.on('PreInit', function () {
            var addReferrerPolicy = function (nodes) {
            ...
            editor.parser.addNodeFilter('iframe', addReferrerPolicy);
            editor.serializer.addNodeFilter('iframe', addReferrerPolicy);
          });
            ...
            onAction: function (_) {
              editor.windowManager.open({
                  ...
                  items: [
                    {
                      type: 'selectbox',
                      name: 'colcount',
                      label: 'Seleccione el diseño',
                      items: [
                        { text: '2 Columnas', value: '2' },
                        { text: '3 Columnas', value: '3' },
                      ],
                    },
                  ],
                },
                ...
                onSubmit: function (api) {
                  ...
                  var colClass =
                    data.colcount === '3' ? 'md:grid-cols-3' : 'md:grid-cols-2';
                  var colsHtml = '';
                  for (var i = 0; i < parseInt(data.colcount); i++) {
                    colsHtml +=
                      '<div class="p-2 border border-dashed border-gray-300">Columna ' +
                      (i + 1) +
                      '</div>';
                  }
  ...
  created() {
    ...
    axios({
      ...
      if (response.data.noticia_idiomas.length > 0) {
        ...
        this.palabras_clave_en =
          response.data.noticia_idiomas[0].palabras_clave;
      }

      if (response.data.link_imagen_cabecera != '') {
        this.preview_image_header_url =
          this.VITE_IMAGES_URL + response.data.link_imagen_cabecera;
      }

      if (response.data.link_imagen_preview != '') {
        this.preview_image_preview_url =
          this.VITE_IMAGES_URL + response.data.link_imagen_preview;
      }

      if (response.data.autor_imagen != '') {
        this.preview_image_author_url =
          this.VITE_IMAGES_URL + response.data.autor_imagen;
      }

      if (response.data.cita_imagen != '') {
        this.preview_image_quote_author_url =
          this.VITE_IMAGES_URL + response.data.cita_imagen;
      }
    });
  },
  methods: {
    register(status = this.activo, redirect = 0) {
      ...
      this.tituloError =
        this.titulo.length > 1 ? '' : 'Este campo es obligatorio';
      this.bajadaNotaError =
        this.bajada_nota.length > 1 ? '' : 'Este campo es obligatorio';
      this.textoError =
        this.texto.length > 1 ? '' : 'Este campo es obligatorio';
      this.imagenCabeceraError =
        this.link_imagen_cabecera.length > 1 ? '' : 'Este campo es obligatorio';
      this.imagenPreviaError =
        this.link_imagen_preview.length > 1 ? '' : 'Este campo es obligatorio';
    ...
    subirImagen(event, campo, preview) {
      // Check if we are in dev/mock mode
      if (this.token === 'dummy-token-for-dev') {
        const file = event.target.files[0];
        if (file) {
          const reader = new FileReader();
          reader.onload = (e) => {
            // Set the preview data (base64)
            this[preview] = e.target.result;
            // Set a dummy filename for the field so validation passes
            this[campo] = file.name;
          };
    ...
    preview() {
      let retVal = confirm(
        "Esta acción pondrá la noticia en estado 'Borrador' para poder previsualizarla. ¿Desea continuar?"
      );
